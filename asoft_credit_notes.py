...
def get_cam():
    ...
    o.save_file("cnm_asoft", cam)
    print(f"Total cnm: {len(cam)}")
...
def get_cad():
    ...
    WHERE CAM.DOC_DATE >= DATEADD(-30 DAY TO CURRENT_TIMESTAMP)
    AND ENTRY_TYPE = 'C'
    ORDER BY CAD.DOC_NO
    """
    ...
    o.save_file("cnd_asoft", cad)
    print(f"Total cnd: {len(cad)}")
