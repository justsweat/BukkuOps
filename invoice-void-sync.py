...
def resync_void_invoices(ivm_asoft, ivd_asoft, iv_added):
    print("Fetching void invoices...")
    void_response = read_void_invoice_list()
    transactions = void_response.get("transactions", [])
    
    if not transactions:
        print("No void invoices found.")
        return

    print(f"Found {len(transactions)} void invoices.")
    ...
    for txn in transactions:
        ...
        print(f"Deleting void invoice {doc_no} from Bukku...")
        o.delete_response(f"sales/invoices/{txn_id}")
        
        if doc_no in iv_added:
            iv_added.remove(doc_no)
            deleted_count += 1
            
    if deleted_count > 0:
        print(f"Removed {deleted_count} invoices from local tracker. Saving...")
        o.save_file("invoices_added", iv_added)
        
    # Re-run bulk creation
    print("Proceeding to recreate invoices...")
...
if __name__ == "__main__":
    ...
    resync_void_invoices(ivm_asoft, ivd_asoft, iv_added)